...
public class ExternalHTableThreadPoolMetricsIT extends BaseHTableThreadPoolMetricsIT {
  ...
  private static ThreadPoolExecutor createThreadPoolExecutor(String threadPoolName) {
    ...
    ThreadFactory threadFactory =
      new ThreadFactoryBuilder().setDaemon(true).setNameFormat(threadPoolName + "-shared-pool-%d")
        .setUncaughtExceptionHandler(Threads.LOGGING_EXCEPTION_HANDLER).build();
    return new HTableThreadPoolWithUtilizationStats(MAX_THREADS_IN_EXTERNAL_THREAD_POOL,
      MAX_THREADS_IN_EXTERNAL_THREAD_POOL, 30, TimeUnit.SECONDS, workQueue, threadFactory,
  ...
  private static Supplier<HTableThreadPoolHistograms>
    getHTableThreadPoolHistogramsSupplier(String threadPoolName) {
    Supplier<HTableThreadPoolHistograms> supplier;
    if (threadPoolName.equals(HISTOGRAM_DISABLED_THREAD_POOL)) {
      ...
      supplier = new Supplier<HTableThreadPoolHistograms>() {
        @Override
        public HTableThreadPoolHistograms get() {
          HTableThreadPoolHistograms hTableThreadPoolHistograms = new HTableThreadPoolHistograms(
            MAX_THREADS_IN_EXTERNAL_THREAD_POOL, QUEUE_CAPACITY_OF_EXTERNAL_THREAD_POOL);
  ...
  public static void setUp() throws Exception {
    ...
    ThreadPoolExecutor nullSupplierExecutor = createThreadPoolExecutor(NULL_SUPPLIER_THREAD_POOL);
    ...
    InstanceResolver.getSingleton(HTableFactory.class, new HTableFactory.HTableFactoryImpl() {
      @Override
      public Table getTable(byte[] tableName, org.apache.hadoop.hbase.client.Connection connection,
